...
begin
  ...
  if p_thumbnail_path is null
    or p_thumbnail_path !~ (
      '^products/'
      || p_product_id::text
      || '/[0-9a-f-]+-thumbnail[.]webp$'
    ) then
    raise exception 'PRODUCT_IMAGE_THUMBNAIL_PATH_INVALID';
  end if;
  ...
  if not exists (
    ...
    raise exception 'PRODUCT_IMAGE_THUMBNAIL_OBJECT_MISSING';
  end if;
...
revoke all on function public.admin_replace_product_image_with_thumbnail(
  uuid,
  text,
  text,
  text,
...
grant execute on function public.admin_replace_product_image_with_thumbnail(
  uuid,
  text,
  text,
  text,
